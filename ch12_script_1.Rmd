...
fac_earn <- cor(x$faculty, x$earnings)
```
```{r}
sat_earn <- cor(x$sat, x$earnings)
...
```{r 4, echo=FALSE}
#ggplot(x, aes(x = earnings, y = faculty, color = sat)) +
 # geom_point() +
  #labs(x = "Earnings", y = "Faculty", color = "SAT") +
  #geom_smooth(method = "lm", se = FALSE)


model <- x %>% 
  lm(earnings ~ faculty + sat, data = .)
...
Estimate Meanings:
Intercept-- if faculty and sat = 0, earnings
Faculty -- for every extra dollar paid to staff, .385 increase in earning
SAT -- for every point increase on the SAT, you add 24.5 dollars to earnings 
...
Units -- Earnings
Treatment -- financial aid applied or financial aid cut
Outcomes -- changes to expected Earnings as a result of treatment

Causal Effect -- as financial aid decreases, earnings also decrease
Must assume financial aid decreasing will in face reduce SAT scores, and that 
SAT scores have causational influences on earnings
Fundamental Problem - can't know for certain what the earnings would be in the
counterfactual
...
**Prompt:** Recall the model we made in Scene 4. Use that model to make a prediction. Consider a school with average faculty salary of $50,000 and average SAT of 1400. What would the the median student's earnings be 10 years from now? Although there are fancier ways of doing this in R --- check out the `predict()` function --- you should use the simple approach of using the regression estimates and then plugging in the appropriate values. Then, what would happen if a change in financial aid were to drop the average SAT score to 1300? What would you forecast earnings to be in that case? Putting the two together: What is the causal effect of the drop in SAT? Do you believe that? That is, if you were the university president, would you believe that forecast?
